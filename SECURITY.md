...
SEAL is designed to defend against the following attack classes. Understanding the threat model helps identify what constitutes a genuine security issue versus a deployment concern.
...
**How SEAL prevents it:** Every tool call is wrapped in a `SealEnvelope` containing a `security_token` (JWT) that cryptographically binds the request to the agent's `SecurityContext`. The `PolicyEngine` evaluates the caller's capabilities on every call — even if the physical channel is the orchestrator itself.
...
**How SEAL prevents it:** The `SecurityContext` establishes an immutable capability boundary at attestation time. Injected instructions cannot expand the agent's `Capability` set at runtime because the `security_token` is signed by the orchestrator's root key and cannot be forged by the agent.

### Tool Server Impersonation

**What it is:** A compromised or rogue tool server claims to be a different (trusted) tool server.

**How SEAL prevents it:** The protocol enforces identity at the agent level (agent → gateway direction). Tool server authenticity is enforced at the physical layer by the Orchestrator Proxy (routing) and TLS (transport). SEAL does not add server-side certificates but is designed to compose with mutual TLS.

### Replay Attacks

**What it is:** An attacker captures a valid `SealEnvelope` and re-submits it to execute a tool call a second time.

**How SEAL prevents it:** Every envelope includes a `timestamp` field (Unix integer). The gateway rejects envelopes whose timestamp falls outside a ±30-second window from the current time. The Ed25519 signature binds the timestamp to the envelope, so an attacker cannot alter it.
...
| **Confidentiality** | Out of scope for SEAL itself; TLS 1.3+ at the transport layer is required |
...
The following are deployment concerns, not SEAL protocol vulnerabilities:
...
SEAL is designed to support the following compliance frameworks. See [docs/security.md](docs/security.md) for the full compliance mapping:
