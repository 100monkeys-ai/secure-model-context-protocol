...
def verify_seal_envelope(envelope: Dict[str, Any], public_key_bytes: bytes, max_age_seconds: int = 30) -> Dict[str, Any]:
    """
    ...
    Args:
        envelope: The incoming JSON payload containing the SealEnvelope.
    ...
    Raises:
        SEALError: If verification fails (invalid format, expired, bad signature).
    """
    ...
    try:
        ...
        raise SEALError("Invalid 'timestamp' format. Expected ISO 8601.", 1000)
    ...
    try:
        ...
        raise SEALError(f"Failed to construct canonical message: {e}", 1000)
        
    # 4. Verify Signature
    try:
        signature_bytes = base64.b64decode(signature_b64)
    except Exception:
        raise SEALError("Invalid base64 encoding for 'signature'.", 1000)
        
    try:
        public_key = Ed25519PublicKey.from_public_bytes(public_key_bytes)
    except ValueError:
        raise SEALError("Invalid Ed25519 public key bytes provided by server configuration.", 3000)
        
    try:
        public_key.verify(signature_bytes, canonical_msg)
    except InvalidSignature:
        raise SEALError("Ed25519 signature verification failed.", 1001)
