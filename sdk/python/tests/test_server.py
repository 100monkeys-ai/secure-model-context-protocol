...
from seal.envelope import create_seal_envelope
from seal.server import verify_seal_envelope
from seal.client import SEALError
...
def test_verify_valid_envelope():
    ...
    envelope = create_seal_envelope(token, payload, key)
    
    verified_payload = verify_seal_envelope(envelope, key.get_public_key_bytes())
    
    assert verified_payload == payload

def test_verify_rejects_modified_payload():
    key = Ed25519Key.generate()
    envelope = create_seal_envelope("token", {"id": 1}, key)
    
    # Tamper with the payload
    envelope["payload"]["id"] = 2
    
    with pytest.raises(SEALError) as exc_info:
        verify_seal_envelope(envelope, key.get_public_key_bytes())
...
def test_verify_rejects_expired_envelope():
    ...
    # create_seal_envelope uses ISO format with Z
    ...
    from seal.envelope import create_canonical_message
...
def test_verify_rejects_wrong_public_key():
    ...
    envelope = create_seal_envelope("token", {"id": 1}, key1)
    
    with pytest.raises(SEALError) as exc_info:
        # Verify with key2 instead of key1
        verify_seal_envelope(envelope, key2.get_public_key_bytes())
